    ...
    with tf.Session() as sess:
        ...
        for i in range(config.MAX_EPOCHS):
            ...
            for step, (x, y) in enumerate(translatorObj.training_iterator(args.batch_size)):
                ...
                summary, cost_on_iter, cur_acc, state, _ = sess.run([p.merged, p.cost, p.average_KL_Divergence, p.final_state, p.train_op],
                                         {p.input_IDs: x,
                                          p.target_probs: y,
                                          p.initial_state: state})
                costs += cost_on_iter
                if step !=0:
                    KL_div = ((step-1)*KL_div + cur_acc)/step
                else:
                    KL_div = cur_acc
                #iters += args.num_steps

                if step % (epoch_size // 10) == 10:
                    print("%.3f, KL div: %.4f, emp. cross entr.: %.3f speed: %.0f samples/s, epoch est. time rem: %.0f" %
                        (step * 1.0 / epoch_size, KL_div, costs / step,
            ...
            for step, (x, y) in enumerate(translatorObj.training_iterator(args.batch_size,testing=True)):
                ...
                if step % (epoch_size // 10) == 10:
                    p.writer.add_summary(summary, i)
            p.writer.flush()
            print("After epoch %d, testing KL div: %.4f, emp. cross entr.: %.3f" %
                (i,KL_div_cnt/step, costs / step))
